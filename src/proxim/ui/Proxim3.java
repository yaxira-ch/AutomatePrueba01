...
//		String strServidor="S432VA3\\BDT" ;	
		...
		Mdl_Variables.Gstr_RutaEvidencia="\\\\XT8260T13AW72\\proxim\\Evidencias";//Ruta en donde se generara las evidencias
				    	...
				    	while (Mdl_Variables.Grst_ListaCasosEjecutar.next()) {	
							...
							if (lbnResultadoEsperado){ 
		    						...
		    						while (Mdl_Variables.Grst_TablaLogin.next()) {	
		    							...
		    							objHeader.menuPrincipal(Mdl_Variables.Gstr_Ignore,"PAGOS", "Administración de Pagos");
			    						if (lbnResultadoEsperado){ 
			    							lbnResultadoEsperado = objPagoMasivoAdministracionPagoBL.administracionPagosMasivos();
			    							for(int f=1;f<=Mdl_Variables.Garr_CasosEjecutar.length-1;f++){
			    								lbnResultadoEsperado = objPagoMasivoAdministracionPagoBL.administracionPagosMasivos();
			    							}
			    						}
		    				    	}
